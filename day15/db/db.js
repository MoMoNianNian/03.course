...
//定义链接数据库的方法
/**
 * @param{function} callback 回调函数，返回链接
 */
...
exports.findAll = function(collection,callback){
  GetConn(function(err,client){
    if(err){
      //如果出错了，就返回提示信息，此时没有返回其他值
      callback('链接数据库失败',null);
    }else{
...
/**
 * 删除数据
 *@param {string} collection 删除的集合名称 
 ...
 * @param {number} [option.start] 查询开始的页面
...
exports.find = function(collection,filter,option,callback){
  ...
  if(arguments.length==2){
    ...
    //不一致，说明filter中取到了值，filter是查询函数
    //将查询条件 设置为默认值
    // if(JSON.stringify(option) !== JSON.stringify({start:1,size:5,order:{time:-1}})){
    //   filter={};
    // }
    //或者：判断filter中是否含有start，size，order中的一个或多个属性，
    //若是一个都没有，说明filter是查询条件
    if(filter.start!=undefined||filter.start!=undefined||filter.start!=undefined){
      filter={};
    }
